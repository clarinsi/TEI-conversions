#!/usr/bin/perl -w
# Convert CoNLL-U to vertical format:
# 1. give metadata to <text> elements, based on the ATT = VAL metadata following a line with # newdoc.id
# 2. output expected token annotations, incl. spacing () and named entities ()
# For further explanation of the output see the corresponding registry file
use utf8;
binmode(STDIN,'utf8');
binmode(STDOUT,'utf8');
binmode(STDERR,'utf8');
$first_doc = 1; #Waiting for very first document
while (<>) {
    chomp;
    # New document
    if (/^# newdoc.id/) {
	unless ($first_doc) { # close opened structures above sentence
	    print "</p>\n";
	    print "</text>\n"
	}
	else {$first_doc = 0}
	$new_doc = 1; # We have the start of a new <text>
	($doc_id) = / = (.+)/;
	undef %meta; # Initialise text metadata
    }
    # New paragraph
    elsif (/^# newpar id/) {
        # Also a new text, output tag with all metadata
	if ($new_doc) { 
	...
	($par_id) = / = (.+)/;
        #Do we want to pre-pend the text id to the par id?
	#$id = $doc_id . "." . $par_id;
	$id = $par_id;
	print "<p id=\"$id\">\n";
    }
	...
	my ($i, $token, $lemma, $UPoS, $XPoS, $UFeats, $head_i, $URel, $EXT, $Local) =
	    split /\t/;
	print STDERR "ERROR: Empty lemma in line: $_\n" if $lemma eq '';
	$line = join("\t",
		     $token,
		     $lemma,
		     $XPoS,
